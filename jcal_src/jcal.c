...
extern const char* fa_jalali_days[];
...
void show_cal0(struct cal_layout* l,struct cal_matrix* m,struct jtm** _j)
{
    unsigned int MAX_LEN = MAX_BUF_SIZE/3;
    char bufY[MAX_LEN], bufM[MAX_LEN], bufD[MAX_LEN];

    if(l->farsi){
        jalali_to_farsi(bufY, MAX_LEN, 0, " ",_j[0]->tm_year);
        if(!l->text_mon)
            jalali_to_farsi(bufM, MAX_LEN, 0, " ",_j[0]->tm_mon+1);
        else snprintf(bufM, MAX_BUF_SIZE, "%s", fa_jalali_months[_j[0]->tm_mon]);
        jalali_to_farsi(bufD, MAX_LEN, 0, " ",_j[0]->tm_mday);
    }else{
        snprintf(bufY, MAX_BUF_SIZE, "%d", _j[0]->tm_year);
        if(!l->text_mon)
            snprintf(bufM, MAX_BUF_SIZE, "%d", _j[0]->tm_mon+1);
        else snprintf(bufM, MAX_BUF_SIZE, "%s", jalali_months[_j[0]->tm_mon]);
        snprintf(bufD, MAX_BUF_SIZE, "%d", _j[0]->tm_mday);
    }

    if(l->color){
        printf("%s%s%s",   TERM_WHITE, bufY, TERM_RESET);
	printf("%s/%s",    TERM_RED,TERM_RESET);
        printf("%s%s%s",   TERM_WHITE, bufM, TERM_RESET);
	printf("%s/%s",    TERM_RED,TERM_RESET);
        printf("%s%s%s", TERM_WHITE, bufD, TERM_RESET);
        if(l->s!=1){
            if(l->farsi)
                printf(" %s%s%s \n", TERM_RED, fa_jalali_days[_j[0]->tm_wday], TERM_RESET);
            else printf(" %s%s%s \n", TERM_RED, jalali_days[_j[0]->tm_wday], TERM_RESET);
        }else printf("\n");
    }else {
        if(l->farsi)
            if(l->text_mon!=1){
                if(l->s!=1)
                    printf("%s / %s / %s   %s \n", bufD, bufM, bufY, fa_jalali_days[_j[0]->tm_wday]);
                else printf("%s / %s / %s\n", bufY, bufM, bufD);
            }else{
                if(l->s!=1)
                    printf("%s   %s  %s  %s\n", fa_jalali_days[_j[0]->tm_wday], bufD, bufM, bufY);
                else printf("%s  %s  %s\n", bufD, bufM, bufY);
            }
        else {
            if(l->text_mon!=1){
                if(l->s!=1)
                    printf("%s / %s / %s   %s \n", bufD, bufM, bufY, jalali_days[_j[0]->tm_wday]);
                else printf("%s / %s / %s\n", bufY, bufM, bufD);
            }else{
                if(l->s!=1)
                    printf("%s   %s  %s  %s\n",  jalali_days[_j[0]->tm_wday], bufD, bufM, bufY);
                else printf("%s  %s  %s\n", bufD, bufM, bufY);
            }
        }
    }
...
void show_0(struct cal_layout* l, struct jtm* j)
{
	struct jtm** _j;struct cal_matrix m;
	l->syear = 1;
	_j = malloc(sizeof(struct jtm*));_j[0] = malloc(sizeof(struct jtm));
